...
//
// See `tests/tabnoteTests.ts` for usage examples.
...
// * stringsUsed - An array of numbers representing which strings have fret positions
function getUnusedStringGroups(numLines: number, stringsUsed: number[]) {
  const stemThrough = [];
  let group = [];
...
export class TabNote extends StemmableNote {
  ...
  drawPositions(): void {
    ...
    for (let i = 0; i < this.positions.length; ++i) {
      ...
      const noteGlyphWidth = this.width;
      const tabX = x + noteGlyphWidth / 2 - el.getWidth() / 2;
