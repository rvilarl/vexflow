...
export class Tuplet extends Element {
  ...
  constructor(notes: Note[], options: TupletOptions = {}) {
    ...
    // We accept beatsOccupied, but warn that it's deprecated:
    // the preferred property name is now notesOccupied.
  ...
  resolveGlyphs(): void {
    ...
    while (n >= 1) {
      this.denomGlyphs.unshift(new Glyph('timeSig' + (n % 10), this.point));
      n = parseInt((n / 10).toString(), 10);
    }
  }
  ...
  getYPosition(): number {
    ...
    // or lowest locations, to form a base yPos
    const firstNote = this.notes[0];
    let yPos;
    if (this.location === Tuplet.LOCATION_TOP) {
      yPos = firstNote.checkStave().getYForLine(0) - Tuplet.metrics.topModifierOffset;
      ...
      for (let i = 0; i < this.notes.length; ++i) {
        ...
        if (note.hasStem() || note.isRest()) {
          ...
          yPos = Math.min(topY, yPos);
          if (modLines > 0) {
            yPos = Math.min(modY, yPos);
          }
        }
      }
    } else {
      ...
      yPos = firstNote.checkStave().getYForLine(lineCheck) + Tuplet.metrics.noteHeadOffset;
    ...
    return yPos + nestedTupletYOffset + yOffset;
  }
  ...
  draw(): void {
    ...
    if (this.ratioed) {
      width = this.denomGlyphs.reduce(addGlyphWidth, width);
    ...
    if (this.ratioed) {
      ...
      this.denomGlyphs.forEach((glyph) => {
        glyph.render(ctx, notationStartX + xOffset, this.yPos + this.point / 3 - 2 + shiftY);
        xOffset += defined(glyph.getMetrics().width);
      });
    }
