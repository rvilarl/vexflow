...
export class StaveHairpin extends Element {
  ...
  static FormatByTicksAndDraw(
    ...
    const lShiftPx = ppt * (options.leftShiftTicks ?? 0);
    const rShiftPx = ppt * (options.rightShiftTicks ?? 0);

    const hairpinOptions = {
      height: options.height,
      yShift: options.yShift,
      leftShiftPx: lShiftPx,
      rightShiftPx: rShiftPx,
      rightShiftTicks: 0,
      leftShiftTicks: 0,
    };
  ...
  renderHairpin(params: {
    ...
    const lShift = this.renderOptions.leftShiftPx;
    const rShift = this.renderOptions.rightShiftPx;

    ctx.beginPath();

    switch (this.hairpin) {
      case StaveHairpin.type.CRESC:
        ctx.moveTo(params.lastX + rShift, yShift + dis);
        ctx.lineTo(params.firstX + lShift, yShift + this.renderOptions.height / 2 + dis);
        ctx.lineTo(params.lastX + rShift, yShift + this.renderOptions.height + dis);
        break;
      case StaveHairpin.type.DECRESC:
        ctx.moveTo(params.firstX + lShift, yShift + dis);
        ctx.lineTo(params.lastX + rShift, yShift + this.renderOptions.height / 2 + dis);
        ctx.lineTo(params.firstX + lShift, yShift + this.renderOptions.height + dis);
        break;
      default:
