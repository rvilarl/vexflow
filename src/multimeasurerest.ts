...
export class MultiMeasureRest extends Element {
  ...
  drawLine(stave: Stave, ctx: RenderContext, left: number, right: number, spacingBetweenLines: number): void {
    ...
    left += padding;
    right -= padding;
    let txt = String.fromCharCode(0xe4ef);
    const el = new Element();
    el.setText(txt);
    el.measureText();
    for (let i = 1; (i + 2) * el.getWidth() + left <= right; i++) {
      txt += String.fromCharCode(0xe4f0);
    }
    txt += String.fromCharCode(0xe4f1);
  ...
  drawSymbols(stave: Stave, ctx: RenderContext, left: number, right: number, spacingBetweenLines: number): void {
    ...
    let txt = '';
    for (let i = 0; i < n4; ++i) {
      txt += String.fromCharCode(0xe4e1) + ' ';
    }
    for (let i = 0; i < n2; ++i) {
      txt += String.fromCharCode(0xe4e2) + ' ';
    }
    ...
    txt = '';
    for (let i = 0; i < n1; ++i) {
      txt += String.fromCharCode(0xe4e3) + ' ';
    }
    elTop.setText(txt);
    elTop.measureText();
