...
export class Beam extends Element {
  ...
  static generateBeams(
    ...
    function getTotalTicks(vfNotes: Note[]) {
      return vfNotes.reduce((memo, note) => note.getTicks().clone().add(memo), new Fraction(0, 1));
    }
  ...
  getSlopeY(x: number, firstXPx: number, firstYPx: number, slope: number): number {
  ...
  calculateFlatSlope(): void {
    ...
    // for flat beams, the slope and yShift are simply 0
    this.slope = 0;
    this.yShift = 0;
  }
