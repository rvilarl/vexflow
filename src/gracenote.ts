...
export class GraceNote extends StaveNote {
  ...
  getStemExtension(): number {
    ...
    ret = (Stem.HEIGHT + ret) * this.getStaveNoteScale() - Stem.HEIGHT;
    return ret;
  }
  ...
  calcBeamedNotesSlashBBox(
    ...
    const protrusionStemDx = Math.cos(slashAngle) * protrusions.stem * scaleX;
    const protrusionStemDy = Math.sin(slashAngle) * protrusions.stem;
    const protrusionBeamDx = Math.cos(slashAngle) * protrusions.beam * scaleX;
    const protrusionBeamDy = Math.sin(slashAngle) * protrusions.beam;
    ...
    const ret = {
      ...
      y1: stemY + slashStemOffset - protrusionStemDy,
      x2: stemX + iPointOnBeam.dx * scaleX + protrusionBeamDx,
      y2: stemY + iPointOnBeam.dy + protrusionBeamDy,
    };
    return ret;
  }
}
